document.addEventListener("DOMContentLoaded", function () {
    ...
    var btn = document.getElementById('createPO');
    var span = document.getElementById('closePOModal');
    ...
    // Initialization of flatpickr for poDeadline
    flatpickr("#poDeadline", { minDate: "today" });

    // JavaScript for adding and removing client fields
    let clientCounter = 1;
    document.getElementById('addClientButton').addEventListener('click', function () {
        clientCounter++;
        const clientsSection = document.getElementById('clientsSection');
        let newField = document.createElement('div');
        newField.className = 'clientField';
        newField.id = 'clientField_' + clientCounter;
        newField.innerHTML = `
              ...
              <span class="deleteClient" onclick="deleteClientField(${clientCounter})">x</span>
          `;
        clientsSection.appendChild(newField);
    });

    function deleteClientField(counter) {
        const fieldToRemove = document.getElementById('clientField_' + counter);
